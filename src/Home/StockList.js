    ...
    const list = stock_arr.map( stock => {
        ...
        <div key={stock.id} className="row pb-1">
            ...
            <div className="col-md-6">
                <small><a className="text-dark" href={"/stock/" + stock.symbol}>{stock.name}</a></small> 
            </div>
            <div className="col-md-2">
                <span className={stock.change > 0 ? "text-success" : "text-danger"}>{numberFormat(stock.change) + "% "}</span> 
            </div>
            <div className="col-md-2">
